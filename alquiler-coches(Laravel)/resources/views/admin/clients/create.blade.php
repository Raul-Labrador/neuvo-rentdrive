...
<div class="main-wrapper">
  ...
  <header class="topbar">
    ...
    <div class="topbar-title">Add Client</div>
  </header>

  <div class="section-header">
    <div>
      <h1 class="section-title">Add Client</h1>
      <p class="section-subtitle">Create a new client (subscriber) in WordPress.</p>
    </div>
  ...
  <form method="POST" action="{{ route('admin.clients.store') }}">
      ...
      <div class="form-card-body">
        <div class="form-grid form-grid-2">
          <div class="form-group form-group-full">
            ...
            <input type="text" name="username" class="form-ctrl @error('username') invalid @enderror" value="{{ old('username') }}" placeholder="e.g. john_doe" required>
            @error('username')<span class="form-error">{{ $message }}</span>@enderror
          </div>
          <div class="form-group">
            <label>First Name</label>
            <input type="text" name="first_name" class="form-ctrl @error('first_name') invalid @enderror" value="{{ old('first_name') }}" placeholder="John">
            @error('first_name')<span class="form-error">{{ $message }}</span>@enderror
          </div>
          <div class="form-group">
            <label>Last Name</label>
            <input type="text" name="last_name" class="form-ctrl @error('last_name') invalid @enderror" value="{{ old('last_name') }}" placeholder="Doe">
            @error('last_name')<span class="form-error">{{ $message }}</span>@enderror
          </div>
          <div class="form-group">
            <label>Email *</label>
            <input type="email" name="email" class="form-ctrl @error('email') invalid @enderror" value="{{ old('email') }}" placeholder="john@example.com" required>
            @error('email')<span class="form-error">{{ $message }}</span>@enderror
          </div>
          <div class="form-group">
            <label>Password *</label>
            <input type="password" name="password" class="form-ctrl @error('password') invalid @enderror" required minlength="6">
    ...
    <div class="form-actions">
      ...
      <button type="submit" class="btn-neuvo"><i class="bi bi-person-plus-fill"></i> Add Client</button>
    </div>
